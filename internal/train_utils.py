...
def compute_robustnerf_mask(
    errors: jnp.ndarray, inlier_threshold: float, config: configs.Config
  ...
  next_inlier_threshold = jnp.quantile(error_per_pixel, config.robustnerf_inlier_quantile)
...
def compute_data_loss(batch, renderings, rays, config):
  ...
  for rendering in renderings:
    ...
    robustnerf_mask, next_threshold = compute_robustnerf_mask(resid_sq, 1.0, config)
    data_losses.append((lossmult * robustnerf_mask * data_loss).sum() / denom)
